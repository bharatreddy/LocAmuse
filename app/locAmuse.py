class BestPlace(object):
    ...
    def yelpOut(self, searchType='quicker' ) :
        ...
        # I am putting them as environment variables
        yelp_keys = {}
        
        yelp_keys['consumer_key'] = os.environ['YELP_CONS_KEY']
        yelp_keys['consumer_secret'] = os.environ['YELP_CONS_SECRET']
        yelp_keys['token'] = os.environ['YELP_TOKEN']
        yelp_keys['token_secret'] = os.environ['YELP_TOKEN_SECRET']
        ...
        # print 'result_json-----YELP',result_json
                            ...
                            for llc in range( len( c1 ) ):
                                if c1[llc] in termArrYelp :
                                    if result_json['businesses'][ly1]['name'] not in nameArrYelp :
                                        ...
                                        if 'snippet_text' in result_json['businesses'][ly1] :
                                            snptTextArrYelp.append( result_json['businesses'][ly1]['snippet_text'] )
                                        else :
                                            snptTextArrYelp.append( 'No text found' )
                                        addressArrYelp.append( result_json['businesses'][ly1]['location']['display_address'] )
                                        # print 'YELP', nameArrYelp[-1], ratArrYelp[-1], revCntArrYelp[-1]
                                        break                  
        ...
        return nameArrYelp, ratArrYelp, revCntArrYelp, snptTextArrYelp, addressArrYelp
    ...
    def foursquareOut(self, latitude=None, longitude=None):
        ...
        # The client object
        # defining them as environment variables (OFCOURSE!!)
        client = foursquare.Foursquare(client_id=os.environ['FS_CLIENT_ID'], \
        ...
        for lfs1 in range( len( allVenues['groups'][0]['items'] ) ) :
            ...
            # print 'FSALL', nameArrFSAll[-1], likesArrFSAll[-1], totChkinArrFSAll[-1], userChkinArrFSAll[-1]
    ...
    def mergeLocs(self, searchType='quicker'):
        ...
        # get data from YELP
        ( nameArrYelp, ratArrYelp, revCntArrYelp, snptTextArrYelp, addressArrYelp ) = self.yelpOut( searchType=searchType )
        # get data from FS
        ...
        addressArrYelp = numpy.array( addressArrYelp )
        # The FS arrays
        ...
        addressArrFSAll = numpy.array( addressArrFSAll )
        ...
        addressArrFSTrnd = numpy.array( addressArrFSTrnd )
        ...
        if len( baysRatYelp ) > 0 :
            maxbaysRatYelp = max( baysRatYelp )
        else :
            maxbaysRatYelp =5.
        baysRatYelp = numpy.array( baysRatYelp/maxbaysRatYelp )
    
        # sometimes the shape of the arrays is wierd, PANDAS requires 1-d arrays and the outputs are not so..check it
        # checking shape of address array
        
        newArr = []
        if len(addressArrYelp.shape) > 1 :
            for j in range(addressArrYelp.shape[0]) :
                newArr.append( addressArrYelp[j][0] )
            newArr = numpy.array( newArr )
            addressArrYelp = newArr

        # checking shape of ratings array
        
        newArr = []
        if len(baysRatYelp.shape) > 1 :
            for j in range(baysRatYelp.shape[0]) :
                newArr.append( baysRatYelp[j][0] )
            newArr = numpy.array( newArr )
            baysRatYelp = newArr

        # checking shape of name array
        newArr = []
        if len(nameArrYelp.shape) > 1 :
            for j in range(nameArrYelp.shape[0]) :
                newArr.append( nameArrYelp[j][0] )
            newArr = numpy.array( newArr )
            nameArrYelp = newArr

        
        # now convert things into PANDAS DataFrame        
        yelpData = pandas.DataFrame( { 'name' : nameArrYelp, 'rating' : baysRatYelp, 'address' : addressArrYelp } )
        # sort the dictionary according to our rating
        yelpData = yelpData.sort( ['rating'], ascending=[True] )
        yelpData = yelpData[::-1]
        # We only need a max of 15 places
        if len(yelpData.index) > 15 :
            yelpData = yelpData[:15]
        ...
        # now convert things into PANDAS DataFrame
        fsAllData = pandas.DataFrame( { 'name' : nameArrFSAll, 'rating' : ratFSAll, 'address' : addressArrFSAll } )
        # sort the dictionary according to our rating
        fsAllData = fsAllData.sort( ['rating'], ascending=[True] )
        fsAllData = fsAllData[::-1]
        # We only need a max of 15 places
        if len(fsAllData.index) > 15 :
            fsAllData = fsAllData[:15]
        ...
        if len(hereNowCntArrFSTrnd) > 0 :
            ...
            # now convert things into PANDAS DataFrame
            fsTRNDData = pandas.DataFrame( { 'name' : nameArrFSTrnd, 'rating' : ratFSTRND, 'address' : addressArrFSTrnd } )
            # sort the dictionary according to our rating
            fsTRNDData = fsTRNDData.sort( ['rating'], ascending=[True] )
            fsTRNDData = fsTRNDData[::-1]
            # We only need a max of 15 places
            if len(fsTRNDData.index) > 15 :
                fsTRNDData = fsTRNDData[:15]
            ...
            # we need to get this here unlike others
            fsTRNDDatTuple = [ tuple(x) for x in fsTRNDData.to_records( index=False ) ]
        else :
            fsTRNDData = None
            fsTRNDDatTuple = (-1,-1, -1)

        # Convert all the data into array of tuples before returning it

        combinedDatTuple = [ tuple(x) for x in combinedData.to_records( index=False ) ]
        yelpDatTuple = [ tuple(x) for x in yelpData.to_records( index=False ) ]
        fsAllDatTuple = [ tuple(x) for x in fsAllData.to_records( index=False ) ]
        # print fsAllDatTuple

            


        return combinedDatTuple, yelpDatTuple, fsAllDatTuple, fsTRNDDatTuple